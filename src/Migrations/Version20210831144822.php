...
final class Version20210831144822 extends AbstractMigration
{
    public function up(Schema $schema): void
    {
        ...
        $this->addSql(
            <<<SQL
            UPDATE motion SET source_id = v.source_id::UUID
            FROM (VALUES {$values}) v(source_id, old_id)
            WHERE motion.source_id = v.old_id::UUID
            SQL,
        );
        $this->addSql(
            <<<SQL
            UPDATE wallet_transaction SET source_id = v.source_id::UUID
            FROM (VALUES {$values}) v(source_id, old_id)
            WHERE wallet_transaction.source_id = v.old_id::UUID
            SQL,
        );
        $this->addSql(
            <<<SQL
            UPDATE customer_transaction SET source_id = v.source_id::UUID
            FROM (VALUES {$values}) v(source_id, old_id)
            WHERE customer_transaction.source_id = v.old_id::UUID
            SQL,
        );
        $this->addSql(
            <<<SQL
            UPDATE part_supply SET source_id = v.source_id::UUID
            FROM (VALUES {$values}) v(source_id, old_id)
            WHERE part_supply.source_id = v.old_id::UUID
            SQL,
        );
    }
}
