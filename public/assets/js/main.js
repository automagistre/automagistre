...
function mobChecker(maxWinWidth){
  if ( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ||
      window.matchMedia('(max-width: ' + maxWinWidth + 'px)').matches || $D.width() < maxWinWidth )
  { return true; }
  else
  { return false; }
}
...
$D.ready(function() {
  ...
  $.fn.scrollToTop = function(){
    ...
    $(this).click(function() {
      $HB.velocity('scroll', {offset: 0, duration: 2000, easing: "easeOutCubic"});
    });
  };
  ...
  $(".js-scroll-to").click(function(e){
    ...
    if ( $target.length ){
      ...
      $HB.velocity('scroll', {offset: targetTop, duration: 1000, easing: "easeOutCubic"});
    }
    return false;
  });
  ...
  $.fn.rippleEffect = function( ratio ) {
    ...
    $that.click(function (e) {
      ...
      if ($ripple.find(".effect").length == 0) $ripple.append("<span class='effect'></span>");
  ...
  $D.on('click', ".js-youtube-init", function () {
    ...
    function youtubeRun(){
      youtubeElem = '<iframe class="video-unit__youtube" width="740" height="480" src="https://www.youtube.com/embed/' +
        youtubeData + '?rel=0&autoplay=1&showinfo=0&rel=0&iv_load_policy=3"  allowfullscreen ></iframe>';
  ...
  $(".js-load-reviews").click(function () {
      reviewsAjaxLoad(".js-load-reviews", "#for-load-reviews", "ajax/more_reviews.php");
  });
  ...
  $("#js-auto-load-reviews").viewportChecker({
    ...
    callbackFunction: function() {
      ...
      reviewsAjaxLoad("#js-auto-load-reviews", "#for-load-reviews", "ajax/more_reviews.php");
    }
  });
  ...
  $("#js-auto-load-blog").viewportChecker({
    ...
    callbackFunction: function() {
      if ( blogLoading ) return;
        blogAjaxLoad("#js-auto-load-blog", "#for-load-blog-cards", "ajax/more_blog-cards.php");
    }
  });
  ...
  $D.on('click', ".selector__list li", function () {
    ...
    $item.closest(".selector__drop").siblings("div").text(selDataTxt).attr({ "data-val": selDataVal });
    $item.closest(".selector__drop").siblings("input").val(selDataVal).trigger("change");
  ...
  $W.load(function () {
    $(".selector").each(function () {
      ...
      if (inputVal) {
        ...
        $selectedItem.addClass("is-active").siblings("li").removeClass("is-active");
      }
      if (selectedText) $that.find(".selector__val").text(selectedText).attr("data-val", inputVal);
  ...
  $(".js-validate").each(function(){
    $(this).validate({
      ...
      submitHandler: function(form) {
        if ( $(form).hasClass("popup-form") )  $(form).hide().siblings(".js-form-ok").show();
        //alert("Submitted!");
        //form.submit();
      }
    });
  ...
  if ( $features.length ) {
    ...
    if ( isMobileView ) {
      ...
      function featuresParr() {
        ...
        TweenLite.to(SECFEAT.$ltMin,  2, { y: thisOffset/8, force3D:true, delay:0.1 });
        TweenLite.to(SECFEAT.$ltBig,  2, { y: thisOffset/5, force3D:true, delay:0.1 });
        TweenLite.to(SECFEAT.$ltBlur, 2, { y: thisOffset/2, force3D:true, delay:0.1 });
        TweenLite.to(SECFEAT.$rtMin,  2, { y: thisOffset/7, force3D:true, delay:0.1 });
        TweenLite.to(SECFEAT.$rtBig,  2, { y: thisOffset/3, force3D:true, delay:0.1 });
        TweenLite.to(SECFEAT.$rtBlur, 2, { y: thisOffset/2, force3D:true, delay:0.1 });
      }
      featuresParr();
  ...
  if ( $expert.length ) {
    ...
    if ( isMobileView ) {
      ...
      function expertParr() {
        ...
        TweenLite.to(SECEXPERT.$lt_01, 2, { y: thisOffset/14, force3D:true, delay:0.1 });
        TweenLite.to(SECEXPERT.$lt_02, 2, { y: thisOffset/12, force3D:true, delay:0.1 });
        TweenLite.to(SECEXPERT.$lt_03, 2, { y: thisOffset/10, force3D:true, delay:0.1 });
        TweenLite.to(SECEXPERT.$lt_04, 2, { y: thisOffset/8, force3D:true, delay:0.1 });
        TweenLite.to(SECEXPERT.$lt_05, 2, { y: thisOffset/6, force3D:true, delay:0.1 });
        TweenLite.to(SECEXPERT.$lt_06, 2, { y: thisOffset/4, force3D:true, delay:0.1 });
        TweenLite.to(SECEXPERT.$lt_07, 2, { y: thisOffset/2, force3D:true, delay:0.1 });

        TweenLite.to(SECEXPERT.$rt_01, 2, { y: thisOffset/12, force3D:true, delay:0.1 });
        TweenLite.to(SECEXPERT.$rt_02, 2, { y: thisOffset/10, force3D:true, delay:0.1 });
        TweenLite.to(SECEXPERT.$rt_03, 2, { y: thisOffset/8, force3D:true, delay:0.1 });
        TweenLite.to(SECEXPERT.$rt_04, 2, { y: thisOffset/6, force3D:true, delay:0.1 });
        TweenLite.to(SECEXPERT.$rt_05, 2, { y: thisOffset/4, force3D:true, delay:0.1 });
        TweenLite.to(SECEXPERT.$rt_06, 2, { y: thisOffset/2, force3D:true, delay:0.1 });
      }
      expertParr();
      $W.scroll( expertParr );
    }
  ...
  if ( $secWorkSlider.length ) {
    ...
    function secWorkfilter( keyClassName, key ){
      ...
      $secWorkSlider.slick("slickUnfilter").slick("slickFilter", "." + keyClassName);
  ...
  $(".js-advantages-toggle").click(function () {
    ...
    if ( $btn.hasClass("is-open") ) {
        $btn.removeClass("is-open").html('Посмотреть преимущества <i class="btn__icon-rt icon_arrow-rt"></i>');
        $info.removeClass("is-open");
    } else {
        $btn.addClass("is-open").html('Скрыть преимущества <i class="btn__icon-rt icon_arrow-rt"></i>');
