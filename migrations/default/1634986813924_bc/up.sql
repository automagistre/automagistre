...
CREATE SCHEMA automagistre;
CREATE TABLEpublic. appeal_calculator (
    id uuid NOT NULL,
...
CREATE VIEW public.appeal_view AS
 SELECT appeal.id,
    appeal.tenant_id,
    appeal.name,
    appeal.type,
   ...
   FROM ((( SELECT appeal_calculator.id,
        ...
        UNION ALL
         SELECT appeal_call.id,
           ...
           FROM public.appeal_call) appeal
     ...
     JOIN public.created_by ON ((created_by.id = appeal.id)));
...
CREATE VIEW public.calendar_entry_view AS
 SELECT e.id,
    e.tenant_id,
    ...
    ceo.order_id
   FROM ((((automagistre.calendar_entry e
     LEFT JOIN public.calendar_entry_deletion ced ON ((e.id = ced.entry_id)))
     LEFT JOIN public.calendar_entry_order ceo ON ((ceo.entry_id = e.id)))
...
CREATE SEQUENCE public.cron_report_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE VIEW public.customer_transaction_view AS
 SELECT ct.id,
    ct.tenant_id,
    ct.operand_id,
   ...
   FROM ((automagistre.customer_transaction ct
     JOIN public.created_by cb ON ((cb.id = ct.id)))
     LEFT JOIN public.wallet_transaction wt ON ((wt.id = ct.source_id)));
...
CREATE VIEW public.customer_view AS
 SELECT o.id,
   ...
   FROM (automagistre.organization o
     LEFT JOIN ( SELECT ct.operand_id AS id,
            sum(ct.amount_amount) AS money
           FROM public.customer_transaction ct
          GROUP BY ct.operand_id, ct.amount_currency_code) balance ON ((balance.id = o.id)))
UNION ALL
 SELECT p.id,
    p.tenant_group_id,
    ...
    'person'::text AS type
   FROM (automagistre.person p
     LEFT JOIN ( SELECT ct.operand_id AS id,
            sum(ct.amount_amount) AS money
           FROM public.customer_transaction ct
          GROUP BY ct.operand_id, ct.amount_currency_code) balance ON ((balance.id = p.id)));
...
CREATE VIEW public.inventorization_part_view AS
 SELECT ip.inventorization_id,
    ip.tenant_id,
    ip.part_id,
    ip.quantity,
    COALESCE(stock.quantity, (0)::bigint) AS in_stock,
    COALESCE(reserved.quantity, (0)::bigint) AS reserved
   FROM ((automagistre.inventorization_part ip
     ...
     LEFT JOIN ( SELECT order_item_part.part_id,
           ...
           FROM (automagistre.reservation
             JOIN public.order_item_part ON ((order_item_part.id = reservation.order_item_part_id)))
          GROUP BY order_item_part.part_id) reserved ON ((reserved.part_id = ip.part_id)));
CREATE VIEW public.inventorization_view AS
 SELECT i.id,
    i.tenant_id,
    cb.created_at,
    cbc.created_at AS closed_at
   FROM (((automagistre.inventorization i
     JOIN public.created_by cb ON ((cb.id = i.id)))
     LEFT JOIN public.inventorization_close ic ON ((i.id = ic.inventorization_id)))
     LEFT JOIN public.created_by cbc ON ((cbc.id = ic.id)));
...
CREATE VIEW public.note_view AS
 SELECT note.id,
    ...
    cb.created_at,
    cb.user_id AS created_by
   FROM ((automagistre.note
     JOIN public.created_by cb ON ((cb.id = note.id)))
     LEFT JOIN public.note_delete ON ((note_delete.note_id = note.id)))
...
CREATE VIEW public.part_analog_view AS
 SELECT pcp.part_id,
    analog.part_id AS analog_id
   FROM (automagistre.part_cross_part pcp
     CROSS JOIN LATERAL ( SELECT pcp2.part_id
           FROM public.part_cross_part pcp2
          WHERE ((pcp.part_cross_id = pcp2.part_cross_id) AND (pcp.part_id <> pcp2.part_id))) analog);
...
CREATE VIEW public.part_view AS
 SELECT part.id,
   ...
   FROM (((((((((((((automagistre.part
     JOIN public.tenant ON (true))
     ...
     LEFT JOIN ( SELECT part_case.part_id,
           ...
           FROM (automagistre.part_case
             LEFT JOIN public.vehicle_model vm ON ((vm.id = part_case.vehicle_id)))
     ...
     LEFT JOIN ( SELECT row_number() OVER (PARTITION BY pra.part_id, pra.tenant_id ORDER BY pra.id DESC) AS rownum,
            ...
            pra.tenant_id
           FROM public.part_required_availability pra) part_required ON (((part_required.part_id = part.id) AND (part_required.rownum = 1) AND (tenant.id = part_required.tenant_id))))
     LEFT JOIN ( SELECT row_number() OVER (PARTITION BY pp.part_id, pp.tenant_id ORDER BY pp.id DESC) AS rownum,
           ...
           FROM public.part_price pp) price ON (((price.part_id = part.id) AND (price.rownum = 1) AND (tenant.id = price.tenant_id))))
     LEFT JOIN ( SELECT row_number() OVER (PARTITION BY pd.part_id, pd.tenant_id ORDER BY pd.id DESC) AS rownum,
           ...
           FROM public.part_discount pd) discount ON (((discount.part_id = part.id) AND (discount.rownum = 1) AND (tenant.id = discount.tenant_id))))
     LEFT JOIN ( SELECT row_number() OVER (PARTITION BY income_part.part_id, income_part.tenant_id ORDER BY income_part.id DESC) AS rownum,
           ...
           FROM public.income_part) income ON (((income.part_id = part.id) AND (income.rownum = 1))))
     LEFT JOIN ( SELECT motion.part_id,
            motion.tenant_id,
            sum(motion.quantity) AS quantity
           FROM public.motion
          GROUP BY motion.part_id, motion.tenant_id) stock ON (((stock.part_id = part.id) AND (tenant.id = stock.tenant_id))))
     LEFT JOIN ( SELECT order_item_part.part_id,
            order_item.tenant_id,
            sum(order_item_part.quantity) AS quantity
           FROM ((automagistre.order_item_part
             JOIN public.order_item ON ((order_item.id = order_item_part.id)))
             LEFT JOIN public.order_close ON ((order_item.order_id = order_close.order_id)))
          WHERE (order_close.* IS NULL)
          GROUP BY order_item_part.part_id, order_item.tenant_id) ordered ON (((ordered.part_id = part.id) AND (tenant.id = ordered.tenant_id))))
     LEFT JOIN ( SELECT order_item_part.part_id,
            reservation.tenant_id,
            sum(reservation.quantity) AS quantity
           FROM (automagistre.reservation
             JOIN public.order_item_part ON ((order_item_part.id = reservation.order_item_part_id)))
          GROUP BY order_item_part.part_id, reservation.tenant_id) reserved ON (((reserved.part_id = part.id) AND (tenant.id = reserved.tenant_id))))
     LEFT JOIN ( SELECT json_agg(json_build_object('supplier_id', sub.supplier_id, 'quantity', sub.quantity, 'updatedAt', sub.updated_at)) AS json,
            sub.tenant_id,
            sub.part_id,
           ...
           FROM ( SELECT part_supply.part_id,
                   ...
                   FROM (automagistre.part_supply
                     LEFT JOIN public.created_by ON ((created_by.id = part_supply.id)))
                  GROUP BY part_supply.part_id, part_supply.tenant_id, part_supply.supplier_id
                 HAVING (sum(part_supply.quantity) <> 0)) sub
          GROUP BY sub.part_id, sub.tenant_id) supply ON (((supply.part_id = part.id) AND (tenant.id = supply.tenant_id))))
     LEFT JOIN ( SELECT pcp.part_id,
            json_agg(pcp2.part_id) FILTER (WHERE (pcp2.part_id IS NOT NULL)) AS parts
           FROM (automagistre.part_cross_part pcp
             LEFT JOIN public.part_cross_part pcp2 ON (((pcp2.part_cross_id = pcp.part_cross_id) AND (pcp2.part_id <> pcp.part_id))))
...
CREATE VIEW public.publish_view AS
 SELECT p.entity_id AS id,
   ...
   FROM (( SELECT row_number() OVER (PARTITION BY publish.entity_id ORDER BY publish.id DESC) AS rownum,
     ...
     JOIN public.created_by cb ON ((cb.id = p.id)))
  WHERE (p.rownum = 1);
...
CREATE VIEW public.salary_view AS
 SELECT es.id,
   ...
   FROM ((((automagistre.employee_salary es
     JOIN public.created_by es_cb ON ((es_cb.id = es.id)))
     JOIN public.employee ON ((employee.id = es.employee_id)))
     LEFT JOIN public.employee_salary_end ese ON ((es.id = ese.salary_id)))
     LEFT JOIN public.created_by ese_cb ON ((ese_cb.id = ese.id)));
CREATE TABLE public.sms (
    id uuid NOT NULL,
    phone_number character varying(35) NOT NULL,
    message character varying(255) NOT NULL,
    date_send timestamp(0) without time zone DEFAULT NULL::timestamp without time zone,
    tenant_id uuid NOT NULL
);
...
CREATE VIEW public.storage_part_view AS
 SELECT part.id
   FROM public.part;
CREATE VIEW public.supply_view AS
...
CREATE VIEW public.wallet_transaction_view AS
 SELECT wt.id,
    wt.tenant_id,
    wt.wallet_id,
    ...
    cb.created_at,
    cb.user_id AS created_by
   FROM ((automagistre.wallet_transaction wt
     JOIN public.created_by cb ON ((cb.id = wt.id)))
     LEFT JOIN public.customer_transaction ct ON ((ct.id = wt.source_id)));
CREATE VIEW public.wallet_view AS
 SELECT w.id,
    ...
    w.use_in_order,
    COALESCE(balance.money, (0)::numeric) AS balance
   FROM (automagistre.wallet w
     LEFT JOIN ( SELECT wt.wallet_id AS id,
            sum(wt.amount_amount) AS money
           FROM public.wallet_transaction wt
          GROUP BY wt.wallet_id, wt.amount_currency_code) balance ON ((balance.id = w.id)));
...
CREATE VIEW public.warehouse_view AS
 WITH RECURSIVE tree(id) AS (
         SELECT root.id,
            wp.warehouse_parent_id AS parent_id,
            0 AS depth,
            root.tenant_id
           FROM (automagistre.warehouse root
...
ALTER TABLE ONLY public.appeal_calculator
    ADD CONSTRAINT appeal_calculator_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.appeal_call
    ADD CONSTRAINT appeal_call_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.appeal_cooperation
    ADD CONSTRAINT appeal_cooperation_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.appeal_postpone
    ADD CONSTRAINT appeal_postpone_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.appeal_question
    ADD CONSTRAINT appeal_question_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.appeal_schedule
    ADD CONSTRAINT appeal_schedule_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.appeal_status
    ADD CONSTRAINT appeal_status_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.appeal_tire_fitting
    ADD CONSTRAINT appeal_tire_fitting_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.calendar_entry_deletion
    ADD CONSTRAINT calendar_entry_deletion_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.calendar_entry_order_info
    ADD CONSTRAINT calendar_entry_order_info_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.calendar_entry_order
    ADD CONSTRAINT calendar_entry_order_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.calendar_entry_schedule
    ADD CONSTRAINT calendar_entry_schedule_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.car
    ADD CONSTRAINT car_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.car_recommendation_part
    ADD CONSTRAINT car_recommendation_part_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.car_recommendation
    ADD CONSTRAINT car_recommendation_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.created_by
    ADD CONSTRAINT created_by_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.cron_job
    ADD CONSTRAINT cron_job_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.cron_report
    ADD CONSTRAINT cron_report_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.employee_salary_end
    ADD CONSTRAINT employee_salary_end_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.google_review_token
    ADD CONSTRAINT google_review_token_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.income_accrue
    ADD CONSTRAINT income_accrue_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.income_part
    ADD CONSTRAINT income_part_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.income
    ADD CONSTRAINT income_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.inventorization_close
    ADD CONSTRAINT inventorization_close_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.inventorization_part
    ADD CONSTRAINT inventorization_part_pkey PRIMARY KEY (inventorization_id, part_id);
ALTER TABLE ONLY public.inventorization
    ADD CONSTRAINT inventorization_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.manufacturer
    ADD CONSTRAINT manufacturer_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.mc_equipment
    ADD CONSTRAINT mc_equipment_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.mc_line
    ADD CONSTRAINT mc_line_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.mc_part
    ADD CONSTRAINT mc_part_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.mc_work
    ADD CONSTRAINT mc_work_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.note_delete
    ADD CONSTRAINT note_delete_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.note
    ADD CONSTRAINT note_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.order_cancel
    ADD CONSTRAINT order_cancel_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.order_close
    ADD CONSTRAINT order_close_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.order_deal
    ADD CONSTRAINT order_deal_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.order_item_group
    ADD CONSTRAINT order_item_group_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.order_item_part
    ADD CONSTRAINT order_item_part_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.order_item
    ADD CONSTRAINT order_item_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.order_item_service
    ADD CONSTRAINT order_item_service_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.order_suspend
    ADD CONSTRAINT order_suspend_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.orders
    ADD CONSTRAINT orders_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.organization
    ADD CONSTRAINT organization_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.part_case
    ADD CONSTRAINT part_case_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.part_cross_part
    ADD CONSTRAINT part_cross_part_pkey PRIMARY KEY (part_cross_id, part_id);
ALTER TABLE ONLY public.part_discount
    ADD CONSTRAINT part_discount_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.part
    ADD CONSTRAINT part_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.part_price
    ADD CONSTRAINT part_price_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.part_required_availability
    ADD CONSTRAINT part_required_availability_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.part_supply
    ADD CONSTRAINT part_supply_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.person
    ADD CONSTRAINT person_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.publish
    ADD CONSTRAINT publish_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.sms_send
    ADD CONSTRAINT sms_send_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.sms_status
    ADD CONSTRAINT sms_status_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.tenant_group
    ADD CONSTRAINT tenant_group_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.tenant
    ADD CONSTRAINT tenant_pkey PRIMARY KEY (id, group_id);
...
ALTER TABLE ONLY public.warehouse_code
    ADD CONSTRAINT warehouse_code_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.warehouse_name
    ADD CONSTRAINT warehouse_name_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.warehouse_parent
    ADD CONSTRAINT warehouse_parent_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.note_delete
    ADD CONSTRAINT fk_22c02b5326ed0855 FOREIGN KEY (note_id) REFERENCES public.note(id);
ALTER TABLE ONLY public.mc_part
    ADD CONSTRAINT fk_2b65786f4d7b7542 FOREIGN KEY (line_id) REFERENCES public.mc_line(id);
ALTER TABLE ONLY public.order_item_part
    ADD CONSTRAINT fk_3db84fc5bf396750 FOREIGN KEY (id) REFERENCES public.order_item(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.income_accrue
    ADD CONSTRAINT fk_425dfa41640ed2c0 FOREIGN KEY (income_id) REFERENCES public.income(id);
ALTER TABLE ONLY public.reservation
    ADD CONSTRAINT fk_42c84955437ef9d2 FOREIGN KEY (order_item_part_id) REFERENCES public.order_item_part(id);
ALTER TABLE ONLY public.inventorization_close
    ADD CONSTRAINT fk_4f6195a04ca655fd FOREIGN KEY (inventorization_id) REFERENCES public.inventorization(id);
ALTER TABLE ONLY public.order_item
    ADD CONSTRAINT fk_52ea1f09727aca70 FOREIGN KEY (parent_id) REFERENCES public.order_item(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.order_item
    ADD CONSTRAINT fk_52ea1f098d9f6d38 FOREIGN KEY (order_id) REFERENCES public.orders(id);
ALTER TABLE ONLY public.employee_salary_end
    ADD CONSTRAINT fk_59455a58b0fdf16e FOREIGN KEY (salary_id) REFERENCES public.employee_salary(id);
ALTER TABLE ONLY public.calendar_entry_order_info
    ADD CONSTRAINT fk_5fbde1c1ba364942 FOREIGN KEY (entry_id) REFERENCES public.calendar_entry(id);
ALTER TABLE ONLY public.income_part
    ADD CONSTRAINT fk_834566e8640ed2c0 FOREIGN KEY (income_id) REFERENCES public.income(id);
ALTER TABLE ONLY public.calendar_entry_schedule
    ADD CONSTRAINT fk_86fdaee3ba364942 FOREIGN KEY (entry_id) REFERENCES public.calendar_entry(id);
ALTER TABLE ONLY public.car_recommendation
    ADD CONSTRAINT fk_8e4baaf2c3c6f69f FOREIGN KEY (car_id) REFERENCES public.car(id);
ALTER TABLE ONLY public.order_close
    ADD CONSTRAINT fk_909ff5398d9f6d38 FOREIGN KEY (order_id) REFERENCES public.orders(id);
ALTER TABLE ONLY public.order_cancel
    ADD CONSTRAINT fk_9599d5a7bf396750 FOREIGN KEY (id) REFERENCES public.order_close(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.order_payment
    ADD CONSTRAINT fk_9b522d468d9f6d38 FOREIGN KEY (order_id) REFERENCES public.orders(id);
ALTER TABLE ONLY public.order_deal
    ADD CONSTRAINT fk_ae0ffb01bf396750 FOREIGN KEY (id) REFERENCES public.order_close(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.mc_line
    ADD CONSTRAINT fk_b37ebc5f517fe9fe FOREIGN KEY (equipment_id) REFERENCES public.mc_equipment(id);
ALTER TABLE ONLY public.mc_line
    ADD CONSTRAINT fk_b37ebc5fbb3453db FOREIGN KEY (work_id) REFERENCES public.mc_work(id);
ALTER TABLE ONLY public.cron_report
    ADD CONSTRAINT fk_b6c6a7f5be04ea9 FOREIGN KEY (job_id) REFERENCES public.cron_job(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.order_suspend
    ADD CONSTRAINT fk_c789f0d18d9f6d38 FOREIGN KEY (order_id) REFERENCES public.orders(id);
ALTER TABLE ONLY public.car_recommendation_part
    ADD CONSTRAINT fk_ddc72d65d173940b FOREIGN KEY (recommendation_id) REFERENCES public.car_recommendation(id);
ALTER TABLE ONLY public.orders
    ADD CONSTRAINT fk_e52ffdee6b20ba36 FOREIGN KEY (worker_id) REFERENCES public.employee(id);
ALTER TABLE ONLY public.order_item_service
    ADD CONSTRAINT fk_ee0028ecbf396750 FOREIGN KEY (id) REFERENCES public.order_item(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.calendar_entry_deletion
    ADD CONSTRAINT fk_f118663dba364942 FOREIGN KEY (entry_id) REFERENCES public.calendar_entry(id);
ALTER TABLE ONLY public.order_item_group
    ADD CONSTRAINT fk_f4bda240bf396750 FOREIGN KEY (id) REFERENCES public.order_item(id) ON DELETE CASCADE;
