...
class NormalizerConfigPass implements ConfigPassInterface
{
    ...
    public function process(array $backendConfig)
    {
        ...
        return $this->normalizeFilterConfig($backendConfig);
    }
            ...
            foreach (['form', 'edit', 'new'] as $view) {
                ...
                foreach ($entityConfig[$view]['fields'] as $fieldName => $fieldConfig) {
                    ...
                    if ($isFormDesignElement && 'tab' === $fieldConfig['type']) {
                        ...
                        if ($isTheFirstGroupElement && $previousTabFieldNumber >= 0 && $fieldNumber > $previousTabFieldNumber + 1) {
        ...
        foreach ($backendConfig['entities'] as $entityName => $entityConfig) {
            foreach (['form', 'edit', 'new'] as $view) {
                foreach ($entityConfig[$view]['fields'] as $fieldName => $fieldConfig) {
                    ...
                    if ($isFormDesignElement && in_array($fieldConfig['type'], [
                        'divider',
                        'group',
                        'section',
                        'tab',
