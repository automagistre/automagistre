...
class PropertyConfigPass implements ConfigPassInterface
{
    ...
    public function process(array $backendConfig)
    {
        ...
        return $this->processFilterConfig($backendConfig);
    }
        ...
        foreach ($backendConfig['entities'] as $entityName => $entityConfig) {
            foreach (['edit', 'list', 'new', 'search', 'show'] as $view) {
                ...
                foreach ($entityConfig[$view]['fields'] as $fieldName => $fieldConfig) {
                    ...
                    if (in_array($view, ['list', 'search', 'show'], true)) {
                    ...
                    if (in_array($view, ['edit', 'new'], true)) {
                        ...
                        if (isset($fieldConfig['help'], $normalizedConfig['type_options']['help'])) {
                    ...
                    if ('avatar' === $normalizedConfig['dataType'] && in_array($view, [
                        'list',
                        'search',
                        'show',
                    ], true)) {
                        // if the user didn't define a label explicitly, hide it but only in 'list' and 'search'
                        if (null === $normalizedConfig['label'] && in_array($view, ['list', 'search'], true)) {
    ...
    private function getFieldFormat($fieldType, array $backendConfig)
    {
        ...
        if (in_array($fieldType, ['bigint', 'integer', 'smallint', 'decimal', 'float'], true)) {
