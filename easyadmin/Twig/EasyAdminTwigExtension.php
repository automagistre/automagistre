...
class EasyAdminTwigExtension extends AbstractExtension
{
    ...
    {
        return [
            new TwigFunction('easyadmin_render_field_for_*_view', [$this, 'renderEntityField'], [
                'is_safe' => ['html'],
    ...
    public function renderEntityField(Environment $twig, $view, $entityName, $item, array $fieldMetadata)
    {
        ...
        try {
            ...
            if (empty($templateParameters['value']) && in_array($fieldMetadata['dataType'], [
                'image',
                'file',
                'array',
                'simple_array',
    ...
    private function addAssociationFieldParameters(array $templateParameters)
    {
        ...
        $isShowActionAllowed = !in_array('show', $targetEntityConfig['disabled_actions'], true);
    ...
    public function getActionsForItem($view, $entityName)
    {
        ...
        return array_filter($viewActions, function ($action) use ($excludedActions, $disabledActions) {
            return !in_array($action['name'], $excludedActions, true) && !in_array($action['name'], $disabledActions, true);
        });
    }
